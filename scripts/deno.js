...
function convertImports(filePath, fileContent) {
  return fileContent.replace(/from '(.*)'/g, (_, importPath) => {
    if (isDir(resolve(filePath.split(sep).slice(0, -1).join(sep), importPath)))
      return `from '${
        importPath.endsWith("/") ? importPath : importPath + "/"
      }mod.ts'`;
    if (importPath === "discord-api-types/v9")
      return "from 'https://raw.githubusercontent.com/discordjs/discord-api-types/main/deno/v9.ts'";
    if (importPath.endsWith(".ts")) return `from '${importPath}'`;
    return `from '${importPath}.ts'`;
  });
}
function readDir(dirPath) {
  if (!existsSync(dirPath.replace(input, output)))
    mkdirSync(dirPath.replace(input, output));
  for (const dir of readdirSync(dirPath, { withFileTypes: true })) {
    if (dir.isDirectory()) {
      readDir(dirPath + sep + dir.name);
    } else {
      if (!dir.name.endsWith(".ts")) continue;
      if (dir.name.endsWith(".deno.ts"))
        denoFiles.push(dir.name.replace(".deno", ""));
