...
import type { RAWDiscordRESTError } from "../types/raw";
...
export default class DiscordRESTError extends Error {
  ...
  constructor(
    ...
    let message =
      "message" in resBody
        ? `${(resBody as { message: string }).message} on ${this.method} ${
            this.path
          }`
        : `Unknown Error on ${this.method} ${this.path}`;
    if ("errors" in resBody) {
      message += `\n ${DiscordRESTError.flattenErrors(
    ...
    for (const fieldName in errors) {
      ...
      if ("_errors" in (errors[fieldName] as object)) {
        messages = messages.concat(
          (
            errors[fieldName] as { _errors: Array<{ message: string }> }
          )._errors.map(
            (err: { message: string }) =>
