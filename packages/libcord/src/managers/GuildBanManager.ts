...
export class GuildBanManager extends Manager {
    ...
    private async _initCache() {
       const data = await this.client.requestHandler.request("GET", GUILD_BANS(this.guild.id))
    ...
    async add(options: BanOptions) {
        const data = await this.client.requestHandler.request("PUT", GET_GUILD_BAN(this.guild.id, options.userId), {
    ...
    async remove(userId: Snowflake) {
        const data = await this.client.requestHandler.request("DELETE", GET_GUILD_BAN(this.guild.id, userId))
    ...
    get(userId: Snowflake) {
        ...
        return new Promise(async (resolve, reject) => {
            const data = await this.client.requestHandler.request("GET", GET_GUILD_BAN(this.guild.id, userId))
